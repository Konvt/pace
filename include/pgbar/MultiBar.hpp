...
namespace pgbar {
  ...
  public:
    ...
    PGBAR__NODISCARD static PGBAR__FORCEINLINE PGBAR__CNSTEVAL details::types::Size size() noexcept
    {
      return sizeof...( Configs ) + 1;
    }
    // Returns the number of progress bars which is running.
    PGBAR__NODISCARD PGBAR__FORCEINLINE details::types::Size active_count() const noexcept
    {
...
# endif
  ...
  PGBAR__NODISCARD PGBAR__FORCEINLINE auto make_multi( prefab::BasicBar<Config, O, M, A>&& bar,
  ...
  PGBAR__NODISCARD PGBAR__FORCEINLINE auto make_multi(
    prefab::BasicBar<Configs, Outlet, Mode, Area>&&... bars ) noexcept( sizeof...( Configs ) == Cnt )
#ifdef __cpp_concepts
    -> MakeMulti_t<prefab::BasicBar<Config, Outlet, Mode, Area>, Cnt>
    requires( Cnt > 0 && sizeof...( Configs ) <= Cnt && details::traits::is_config<Config>::value
              && ( std::is_same_v<Config, std::decay_t<Configs>> && ... ) )
...
struct std::tuple_element<I, pgbar::MultiBar<Bs...>> : pgbar::details::traits::TypeAt<I, Bs...> {};
