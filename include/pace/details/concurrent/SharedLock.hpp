#ifndef PACE_SHARED_LOCK
#define PACE_SHARED_LOCK
  ...
  namespace details {
    namespace concurrent {
      ...
      public:
        ...
        PACE__FORCEINLINE void lock() & noexcept
        {
          PACE__TRUST( is_owner_ == false );
          mtx_->lock_shared();
          is_owner_ = true;
        }
        PACE__FORCEINLINE bool try_lock() & noexcept
        {
          PACE__TRUST( is_owner_ == false );
          is_owner_ = mtx_->try_lock_shared();
          return is_owner_;
        }
        PACE__FORCEINLINE void unlock() & noexcept
        {
          PACE__TRUST( is_owner_ == true );
          mtx_->unlock_shared();
          is_owner_ = false;
        }
      };
