...
#undef PACE__UNPAKING
      ...
      protected:
        ...
        PACE__CXX20_CNSTXPR Filler( traits::TypeSet<Options...> tag ) : Base( tag )
        {
          ...
          if PACE__CXX17_CNSTXPR ( !traits::TpContains<OptionSet, option::Filler>::value )
            unpack( *this, config::provide_for<Derived, option::Filler>() );
          if PACE__CXX17_CNSTXPR ( !traits::TpContains<OptionSet, option::FillerForecolor>::value )
            unpack( *this, config::provide_for<Derived, option::FillerForecolor>() );
          if PACE__CXX17_CNSTXPR ( !traits::TpContains<OptionSet, option::FillerBackcolor>::value )
...
#define PACE__METHOD( OptionName, ParamName, ReturnType, Operation ) \
  ...
  return static_cast<ReturnType>( *this )
        ...
        Derived& filler( std::string _filler ) & { PACE__METHOD( Filler, _filler, Derived&, std::move ); }
        Derived&& filler( std::string _filler ) && { PACE__METHOD( Filler, _filler, Derived&&, std::move ); }
#ifdef __cpp_lib_char8_t
        Derived& filler( charcodes::U8StringView _filler ) & { PACE__METHOD( Filler, _filler, Derived&, ); }
