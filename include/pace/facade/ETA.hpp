...
namespace pace {
  ...
  namespace facade {
    ...
    class ETA : public Base {
      ...
      PACE__FORCEINLINE friend PACE__CXX20_CNSTXPR void unpack( ETA& self, option::ETAFormat&& val )
      {
...
# define PACE__METHOD( ParamName, ReturnType )                                                             \
   std::lock_guard<details::concurrent::SharedMutex> lock { this->rw_mtx_ };                               \
   unpack( *this,                                                                                          \
           option::ETAFormat( { reinterpret_cast<const char*>( ParamName.data() ), ParamName.size() } ) ); \
   return static_cast<ReturnType>( *this )
